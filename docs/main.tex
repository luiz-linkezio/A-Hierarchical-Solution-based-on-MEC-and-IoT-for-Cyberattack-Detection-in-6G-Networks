...
nós de borda surgem como alternativa viável: treinados em dados históricos de
...
Este trabalho endereça essas limitações propondo e implementando um IDS
...
falsos positivos, FPR, e de falsos negativos, FNR) e de custo (CPU, RAM,
\textit{throughput} de alertas e energia). Nos resultados mais relevantes, o IDS
binário alcançou FPR de 0\% sobre um baseline benigno limpo, com precisão de
89,3\% e detecção de pelo menos 99\% dos ataques de força bruta, web,
\textit{Man-in-the-Middle} (MITM) e \textit{spoofing}; a Fase~2 acrescentou
apenas 50~MB de RAM, sem overhead de CPU ou energia, classificou corretamente o
reconhecimento de rede (F1 = 0,976) e delimitou empiricamente onde as 55
\textit{features} por fluxo deixam de separar os tipos de ataque.
...
\toprule
\textbf{Dimensão}
  & \shortstack{\textbf{Houichi}\\\textbf{et~al.}}
  & \shortstack{\textbf{Chen}\\\textbf{et~al.}}
  & \shortstack{\textbf{Gyamfi e}\\\textbf{Jurcut}}
  & \shortstack{\textbf{Yang}\\\textbf{et~al.}}
  & \shortstack{\textbf{Aldaej}\\\textbf{et~al.}}
  & \shortstack{\textbf{Chinna-}\\\textbf{samy et~al.}}
  & \shortstack{\textbf{Este}\\\textbf{trabalho}}
\\ \midrule
Deploy em borda real  & Não & Não & \multirow{5}{*}{\shortstack{\textit{revisão de}\\\textit{literatura}}} & Parcial & Sim     & Não & \textbf{Sim} \\
Avaliação online      & Não & Não &  & Não     & Não     & Não & \textbf{Sim} \\
Custo no dispositivo  & Não & Não &  & Não     & Não     & Não & \textbf{Sim} \\
Multi-estágio         & Não & Não &  & Sim     & Sim     & Não & \textbf{Sim} \\
Foco 6G               & Não & Não &  & Não     & Não     & Sim & \textbf{Sim} \\
\bottomrule
\end{tabular}}
\end{table}
...
\textit{testbed} com tráfego de ataque real. As subseções seguintes detalham cada etapa na
ordem da figura: a arquitetura hierárquica (Seção~\ref{subsec:arquitetura}), o
pipeline de dados (Seção~\ref{subsec:pipeline}), o mapeamento de rótulos
...
\begin{enumerate}
  ...
  \item \textbf{Fase~2 — Classificador multiclasse (nó de borda ou host MEC):}
    ...
    host MEC dedicado quando o nó de borda for muito restrito.
\end{enumerate}

O encadeamento das duas fases tem efeito direto sobre o custo de operação. Cada
fluxo extraído pelo \textit{netflower} é primeiro submetido à Fase~1; se a
probabilidade de ataque estimada não atinge o limiar de decisão, o fluxo é
...
sinalizada como suspeita aciona a Fase~2, que lhe atribui um rótulo de tipo. Como
a inferência da Fase~2 ocorre em série após a Fase~1 e sobre um número reduzido
de fluxos, seu custo marginal é pequeno e amortizado pelo rastreamento de fluxos,
que já domina o tempo de processamento (Seção~\ref{sec:resultados}). Neste
trabalho, ambas as fases foram avaliadas executando no próprio nó de
...
Uma vez que os ataques DDoS e DoS visam exaurir recursos e seus fluxos de rede (alta taxa de pacotes, curta duração, anomalias de flags) são estatisticamente similares, manter duas classes separadas pode adicionar ruído sem ganho discriminativo significativo no classificador. Nesse sentido, após análise das distribuições das duas classes, realizou-se a unificação delas, reduzindo o espaço de classes total de 9 para 8 (7 de ataque + 1 benigno), simplificando o problema da Fase~2. Para isso, aplicou-se $\texttt{LABEL\_MAP} = \{\text{"ddos"} \rightarrow \text{"dos"}\}$ antes do treinamento.   
...
\begin{tikzpicture}[
  ...
  \draw[attackarr] (atk.east) --
        node[pos=0.40, below=3pt, align=center, font=\scriptsize, text=black]
...
\end{tikzpicture}}
\caption{Ambiente de experimentação: elementos de software (orquestrador, gerador de
ataques, IDS e cálculo de métricas) e os \textit{loggers} de cada lado (relatório de
ataques no PC e log do IDS na VIM~4), sobre o hardware conectado por rede local isolada.
O \texttt{ids\_metrics.py} cruza o relatório de ataques (\textit{ground truth}) com o log
do IDS coletado da VIM~4 para calcular as métricas.}
\label{fig:ambiente}
\end{figure}
...
O \texttt{attack\_generator.py} executa 8
...
implantação que condicionam esses números (Seção~\ref{subsec:res_artefatos}); e,
por fim, confrontam-se os dois pipelines em uma comparação direta de detecção,
recursos, \textit{throughput} e energia (Seção~\ref{subsec:res_comp}).

Antes dos números, convém precisar a semântica das métricas reportadas e por que
são adequadas a um IDS. A precisão é a fração de alertas que correspondem
a ataques reais (TP/(TP+FP)); precisão baixa significa muitos alarmes falsos, que
sobrecarregam o operador. A revocação (ou taxa de verdadeiros positivos,
TPR) é a fração de ataques efetivamente detectados (TP/(TP+FN)); revocação baixa
significa ataques que passam despercebidos. As duas se complementam, e o
F1 é sua média harmônica, um resumo único que só é alto quando ambas o
são. A taxa de falsos positivos (\textit{False Positive Rate}, FPR) é a
fração de tráfego benigno classificado como ataque (FP/(FP+TN)); em um IDS é
crítica, pois um FPR alto inviabiliza a operação ao afogar a equipe em alarmes
sobre tráfego legítimo. A taxa de falsos negativos (\textit{False
Negative Rate}, FNR = 1\,$-$\,revocação) mede o complemento, os ataques perdidos.
...
O número que mais importa para operar o IDS é o FPR medido sobre o baseline
...
\noindent$^{b}$ Estimativa calibrada, não medição direta: o VIM~4 não
expõe interfaces de telemetria de energia em software (RAPL, \textit{Running
Average Power Limit}, ou o sensor INA226). O método e suas limitações são
